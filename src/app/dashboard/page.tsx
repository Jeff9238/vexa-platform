...
import { ArrowLeft, Trash2, Wallet, Plus, Pencil, MapPin, Eye, MessageCircle, Phone, TrendingUp, Car, Home, CheckCircle2, CircleDashed, MessageSquare, Bell, X } from "lucide-react";
import { deleteListing, toggleListingStatus, deleteSavedSearch, getSavedSearches } from "../actions"; 
...
export default async function Dashboard() {
  ...
  // Fetch Saved Searches (User Retention)
    ...
    <div className={`min-h-screen bg-[#0a0a0a] text-white ${sansFont.className} p-6 pt-28`}>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
            ...
            {/* 2. Performance & Chat Card */}
            <div className="bg-neutral-900 border border-white/10 rounded-3xl p-6">
                <h3 className="text-gray-500 text-xs font-bold uppercase tracking-widest mb-6">Performance</h3>
                ...
                {/* RESTORED: Inbox Link */}
                <Link href="/chat" className="p-4 bg-black hover:bg-neutral-800 transition-colors rounded-2xl border border-white/5 flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="text-xs text-gray-500">View messages</div>
                        </div>
                    </div>
            ...
            {/* 3. Saved Alerts Card (RESTORED) */}
            <div className="bg-neutral-900 border border-white/10 rounded-3xl p-6">
                <h3 className="text-gray-500 text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2">
                    <Bell size={14}/> Saved Alerts
                </h3>
                <div className="space-y-3">
                    {savedSearches.length === 0 ? (
                        <p className="text-sm text-gray-500 italic">No saved alerts yet.</p>
                    ) : savedSearches.map(s => (
                        <div key={s.id} className="flex items-center justify-between bg-black p-3 rounded-xl border border-white/5">
                            <div className="min-w-0">
                                <p className="text-white font-bold text-sm truncate">{s.query || s.type}</p>
                                <p className="text-[10px] text-gray-500 truncate">
                                    {s.state ? s.state : 'All Malaysia'} • {s.listingCategory || 'Any'}
                                </p>
                            </div>
                            <form action={async () => { 'use server'; await deleteSavedSearch(s.id) }}>
                                <button className="text-gray-600 hover:text-red-500 p-1"><X size={14}/></button>
                            </form>
                        </div>
                    ))}
                </div>
        ...
        <div className="lg:col-span-2 space-y-12">
            ...
            <div>
                ...
                <div className="space-y-4">
                    {properties.length === 0 ? (
                        <div className="p-8 border border-white/5 bg-white/5 rounded-2xl text-center text-gray-500 text-sm">No properties listed.</div>
                    ) : properties.map((item) => (
            ...
            <div>
                ...
                <div className="space-y-4">
                    {vehicles.length === 0 ? (
                        <div className="p-8 border border-white/5 bg-white/5 rounded-2xl text-center text-gray-500 text-sm">No vehicles listed.</div>
                    ) : vehicles.map((item) => (
                        <DashboardListingCard key={item.id} item={item} />
                    ))}
                </div>
        ...
        <div className={`border p-6 rounded-2xl flex flex-col gap-6 group transition-all ${isSold ? 'bg-neutral-900/50 border-white/5' : 'bg-neutral-900 border-white/5 hover:border-white/20'}`}>
            <div className="flex gap-4">
                <div className="relative w-24 h-24 bg-neutral-800 rounded-xl overflow-hidden flex-shrink-0">
                    <Image src={item.images ? item.images.split(',')[0] : 'https://via.placeholder.com/150'} alt="Thumb" fill className={`object-cover ${isSold ? 'grayscale opacity-50' : ''}`} />
                    {isSold && <div className="absolute inset-0 flex items-center justify-center"><span className="text-[10px] font-bold text-white uppercase tracking-widest bg-black/80 px-2 py-1 rounded border border-white/10">Sold</span></div>}
                </div>
                ...
                <div className="flex flex-col gap-2">
                    <form action={async () => { 'use server'; await toggleListingStatus(item.id, isSold ? 'ACTIVE' : 'SOLD') }}>
                        <button className={`p-2 rounded-lg transition-all text-center flex items-center gap-2 text-xs font-bold w-full justify-center ${isSold ? 'bg-green-900/20 text-green-500 border border-green-900/50' : 'bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white'}`} title={isSold ? "Mark as Active" : "Mark as Sold"}>
                            {isSold ? <CheckCircle2 size={16}/> : <CircleDashed size={16}/>} {isSold ? "SOLD" : "ACTIVE"}
                        </button>
                    </form>
                    <Link href={`/edit/${item.id}`} className="p-2 bg-white/5 text-gray-400 rounded-lg hover:bg-white/10 hover:text-white transition-all text-center" title="Edit"><Pencil size={16}/></Link>
                    <form action={async () => { 'use server'; await deleteListing(item.id) }}>
                        <button className="p-2 bg-red-900/10 text-red-500 rounded-lg hover:bg-red-900/30 transition-all w-full flex justify-center" title="Delete"><Trash2 size={16}/></button>
                    </form>
                </div>
            </div>
            <div className="flex items-center justify-between pt-4 border-t border-white/5">
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2 text-sm text-gray-400"><Eye size={16}/> <span className="font-bold text-white">{item.views}</span></div>
                    <div className="flex items-center gap-2 text-sm text-gray-400"><MessageCircle size={16}/> <span className="font-bold text-white">{item.whatsappClicks}</span></div>
                    <div className="flex items-center gap-2 text-sm text-gray-400"><Phone size={16}/> <span className="font-bold text-white">{item.callClicks}</span></div>
                </div>
