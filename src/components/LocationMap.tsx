...
import { ShoppingBag, Utensils, Coffee, School, Loader2, MapPin, Trees, Dumbbell } from 'lucide-react';
...
// NEW PROP: hideNearby
export default function LocationMap({ lat, lng, hideNearby = false }: { lat: number, lng: number, hideNearby?: boolean }) {
  ...
  useEffect(() => {
    if (!isLoaded || !window.google || hideNearby) return; // Skip if hidden
    ...
    <div className="flex flex-col h-full gap-6">
        ...
        <div className="h-[350px] w-full rounded-2xl overflow-hidden shadow-2xl border border-white/10 relative">
            ...
            >
                ...
                {/* Only show nearby pins if not hidden */}
                {!hideNearby && places.map((place) => (
                    <Marker 
                        key={place.place_id}
        ...
        {/* NEARBY LIST (Conditionally Rendered) */}
        {!hideNearby && (
