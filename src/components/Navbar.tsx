...
import { Menu, X, LayoutDashboard, Plus, Heart, Settings, Car, Home, Search, ArrowLeft, MessageSquare, LogIn } from "lucide-react"; 
            ...
            {mobileSearchOpen ? (
                ...
                <>
                    ...
                    <div className="flex items-center justify-end gap-3 z-20 flex-shrink-0">
                        ...
                        {/* --- FIXED LOG IN BUTTON (Visible on Mobile) --- */}
                        <SignedOut>
                            <SignInButton mode="modal">
                                <button className="flex items-center gap-2 text-gray-300 font-bold text-xs hover:text-white transition-colors uppercase tracking-widest px-3 py-2 border border-white/10 rounded-full hover:bg-white/5">
                                    <LogIn size={16} className="md:hidden"/> {/* Icon for Mobile */}
                                    <span className="hidden md:inline">Log in</span> {/* Text for Desktop */}
                                </button>
                            </SignInButton>
                        ...
                        <SignedIn>
                            ...
                            <Link href="/chat" className="relative flex items-center justify-center w-10 h-10 rounded-full text-gray-400 hover:text-white hover:bg-white/5 transition-all">
                                ...
                                {unreadCount > 0 && <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-black"></span>}
                            </Link>
                            <Link href="/dashboard" className="hidden md:flex items-center gap-2 text-gray-300 font-bold text-sm hover:text-white transition-colors bg-white/5 px-4 py-2.5 rounded-full border border-white/5 hover:border-white/20"><LayoutDashboard size={16}/> <span className="hidden lg:inline">Dashboard</span></Link>
    ...
    {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-[#0a0a0a] pt-24 px-6 animate-in slide-in-from-top-10 duration-200 md:hidden">
            <div className="flex flex-col gap-6 text-xl font-bold text-gray-300">
                ...
                {/* --- FIXED MOBILE MENU LOG IN --- */}
                <SignedOut>
                    <div className="mt-8 pt-6 border-t border-white/10 space-y-4">
                        <p className="text-xs text-gray-500 uppercase tracking-widest font-bold mb-2">Join VEXA</p>
                        <SignInButton mode="modal">
                            <button className="block w-full bg-white/10 text-white text-center py-4 rounded-2xl font-bold hover:bg-white/20 transition-all flex items-center justify-center gap-2">
                                <LogIn size={20}/> Log In / Sign Up
                            </button>
                        </SignInButton>
                        <Link href="/post" onClick={() => setMobileMenuOpen(false)} className="block w-full bg-blue-600 text-white text-center py-4 rounded-2xl font-bold flex items-center justify-center gap-2">
                           <Plus size={20}/> Post Free Ad
                        </Link>
                    </div>
                </SignedOut>
