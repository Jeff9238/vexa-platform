...
function DeveloperDashboardContent() {
  ...
  const initializeFirebase = useCallback(async () => {
    try {
        ...
        setDb(dbInstance);
        await checkUser(dbInstance); // Await this to ensure loading state is managed
    } catch (e) { 
        console.error(e);
  ...
  const checkUser = async (database: any) => {
      ...
      try {
          ...
          if (userData.role === 'developer') {
              ...
              await fetchMyProjects(currentUser.uid, database); // Await fetch
          } else {
  ...
  const fetchMyProjects = async (userId: string, database: any) => {
      try {
          // Note: This query requires an index on agentId + type.
          // If index missing, it fails. We catch and stop loading.
          const q = query(
          ...
          // If index error, we might show empty for now, but stop loading
      } finally { 
          setLoading(false); 
      }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!file) return;
      if (formData.images.length >= 10) { alert("Maximum 10 images."); return; }
      setProcessingImage(true);
      try {
          const base64 = await compressImage(file);
          const newImages = [...formData.images, base64];
          setFormData({ ...formData, images: newImages, coverImage: formData.coverImage || base64 });
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20 font-sans">
        ...
        <div className="bg-white border-b border-slate-200 px-4 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
            ...
            <button onClick={() => { setIsCreating(true); setIsEditing(false); }} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 font-bold text-sm"><Plus size={18} /> New Project</button>
        </div>
                        ...
                        <div key={project.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                            ...
                            <div className="p-4">
                                ...
                                <div className="flex justify-between items-center border-t border-slate-100 pt-4">
                                    <span className="text-xs text-slate-400">{project.views || 0} Views</span>
                                    <button onClick={() => { setFormData(project); setEditingId(project.id); setIsEditing(true); setIsCreating(true); }} className="text-blue-600 text-sm font-bold flex items-center gap-1"><Pencil size={14}/> Edit</button>
                                </div>
                            </div>
                        </div>
                    ))}
                        ...
                        <div>
                            ...
                            <div className="flex gap-2 overflow-x-auto pb-2">
                                {formData.images.map((img, idx) => (
                                    <img key={idx} src={img} className="w-20 h-20 object-cover rounded-lg border" />
                                ))}
                                <label className={`w-20 h-20 flex flex-col items-center justify-center border-2 border-dashed rounded-lg cursor-pointer hover:bg-slate-50 ${processingImage ? 'opacity-50' : ''}`}>
